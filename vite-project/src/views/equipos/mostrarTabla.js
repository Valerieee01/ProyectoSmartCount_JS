...
export const ITEMS_PER_PAGE_EQUIPOS = 4; // Ajusta según la necesidad
export let currentPageEquipos = 1;       // Página actual para equipos
...
export const cargar_tabla_con_filtros_equipos = async (tabla, numeroEquipoFilter = null, placaFilter = null) => {
    ...
    console.log(`[cargar_tabla_con_filtros_equipos] Filtros activos: Numero='${currentNumeroEquipoFilter}', Placa='${currentPlacaFilter}'`);
 ...
 */
export const crearFilaEquipo = ({id_equipo, numero_equipo, placa, descripcion, id_cliente, nombre_cliente }, tabla) => {
    const tBody = tabla.querySelector("tbody");
    ...
    tdFechaRegistro.textContent = new Date(equipo.fecha_registro).toLocaleDateString(); // Formatear la fecha
...
export const agregarEventosBotonesEquipos = async (tabla) => {
    ...
    tabla.addEventListener('click', async (e) => {
        if (e.target.classList.contains('eliminar')) {
            ...
            if (confirm(`¿Estás seguro de eliminar el Equipo con ID ${equipoId}?`)) {
                try {
                    ...
                    await eliminarEquipoPorId(equipoId); 
        ...
        if (e.target.classList.contains('editar')) {
            ...
            location.hash = `#editarequipo/${equipoId}`; // Redirige a la vista de edición de equipo
        }
    });
};