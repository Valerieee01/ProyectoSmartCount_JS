...
export const cargar_tabla = async (tabla) => {
    try {
        ...
        if (allEmpleados.length === 0) { // Si aún no se han cargado, cárgalos
            const response = await listarEmpleados();
            allEmpleados = response.data; // Asume que los datos están en 'response.data'
            console.log("Todos los clientes cargados:", allEmpleados);
        }
        ...
        const empleadosToDisplay = allEmpleados.slice(startIndex, endIndex);

        empleadosToDisplay.forEach(cliente => {
...
const renderPaginator = (tabla) => {
    const totalPages = Math.ceil(allEmpleados.length / ITEMS_PER_PAGE);
